...
function getPolicy(world, thetas, tilings)
    local function getActivations(state, tilings, action)
        ...
        for _, tiling in pairs(tilings) do
            ...
            phi = (phi and (phi + activations)) or activations
        end
    ...
    local function getBestAction(state)
        ...
        return argmax(world:getActions(state), qSubA)
    end
...
function qLearning(world, theta, tilings)
    ...
    local function runEpisode(state, tilings)
        local eligibilityTrace = vectorForm(theta*0)
        local reward = 0

        local function getQSubA(world, state, tilings)
            local qSubA = {}
        ...
        while (world:get(state).type ~= "terminal") do
            ...
            local qSubA = getQSubA(world, state, tilings)
            action = argmax(world:getActions(state), qSubA)
            ...
            local delta = reward - qSubA[action]
            eligibilityTrace = eligibilityTrace + getActivations(state, tilings)--, action)

            if world:get(state).type == "terminal" then
                theta = theta + alpha*delta*eligibilityTrace
                break
            end

            qSubA = getQSubA(world, state, tilings)
            delta = delta + gamma*max(world:getActions(state), qSubA)
            theta = theta + alpha*delta*eligibilityTrace -- Sam had phi here, as well, but I don't think that that's necessary
            eligibilityTrace = gamma*lambda*eligibilityTrace
        end
...
function getThetas(world, tiling)
    local thetas = {}
    for i = 1, sum(map(tiling, function(row, ind) return sum(map(row, function(col, ind2) return #world:getActions({ind, ind2}) end)) end)) do
...
testWorld = make2DMaze({
    {panel("wall"), panel("terminal", 10000)},
